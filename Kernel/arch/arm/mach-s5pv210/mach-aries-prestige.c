...
//yuezengzhen for W899GB
	/* CAM_VGA_nSTBY - GPB(0) */
#ifdef CONFIG_MACH_S5PC110_PRESTIGE
        err = gpio_request(GPIO_CAM_VGA_nSTBY, "GPC1");
        if (err) {
...
//yuezengzhen for W899GB
	...
	/* Turn CAM_ISP_HOST_2.8V on */ //sjlee_0125 (H/W timing problem)
...
#ifdef CONFIG_MACH_S5PC110_PRESTIGE
	gpio_direction_output(GPIO_CAM_IO_EN, 1); //sjlee_0125 (H/W timing problem)
	gpio_set_value(GPIO_CAM_IO_EN, 0);
	gpio_free(GPIO_CAM_IO_EN);
#else
	gpio_free(GPIO_GPB7);
...
#else
#if 0 // it is aleady done on above code
