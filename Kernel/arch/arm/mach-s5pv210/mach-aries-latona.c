...
// CONFIG_CTC_CARD_KERNEL
struct device *slot_switch_dev;
EXPORT_SYMBOL(slot_switch_dev);
//
...
// CONFIG_CTC_CARD_KERNEL
void (*cdma_slot_switch_handler)(int slot_switch);
EXPORT_SYMBOL(cdma_slot_switch_handler);

void (*gsm_slot_switch_handler)(int slot_switch);
EXPORT_SYMBOL(gsm_slot_switch_handler);
//
...
static unsigned int camera_gpio_table[] =
{
        //GPE0 ---------
        S5PV210_GPE0(0),
        S5PV210_GPE0(1),
...
static void aries_switch_init(void)
{
	...
	//START CONFIG_CTC_CARD_KERNEL
	slot_switch_dev = device_create(sec_class, NULL, 0, NULL, "slot_switch");
	if (IS_ERR(slot_switch_dev))
		pr_err("Failed to create device(switch)!\n");
	//END CONFIG_CTC_CARD_KERNEL
...
static struct s3cfb_lcd s6e63m02 = {
	.width = 480,
	.height = 800,
	.p_width = 52,
	.p_height = 86,
...
//yuezengzhen for W899GB
#if defined(CONFIG_MACH_S5PC110_ARIESCT) || defined(CONFIG_MACH_S5PC110_PRESTIGE) || defined (CONFIG_MACH_S5PC110_GALAXY) || defined (CONFIG_MACH_S5PC110_LATONA) //TEMP for compile
...
struct platform_device sec_device_dpram_gsm = {
	.name	= "dpram1-gsm",
	.id 	= -1,
};
...
static void tl2796_cfg_gpio(struct platform_device *pdev)
{
	...
	/* DISPLAY_SUB_CS */
	s3c_gpio_cfgpin(GPIO_DISPLAY_SUB_CS, GPIO_DISPLAY_SUB_CS_AF);
	s3c_gpio_setpull(GPIO_DISPLAY_SUB_CS, S3C_GPIO_PULL_NONE);
	
#endif  // CONFIG_DUAL_LCD
}
...
void lcd_cfg_gpio_early_suspend(void)
{
	...
	/* DISPLAY_SUB_CS */
	s3c_gpio_cfgpin(GPIO_DISPLAY_SUB_CS, S3C_GPIO_OUTPUT);
	s3c_gpio_setpull(GPIO_DISPLAY_SUB_CS, S3C_GPIO_PULL_NONE);
	s3c_gpio_setpin(GPIO_DISPLAY_SUB_CS, 0);	
...
#if defined(CONFIG_DUAL_LCD)
	err = gpio_request(GPIO_SLCD_RST, "SLCD_RST");
	if (err) {
		printk(KERN_ERR "failed to request GPIO_SLCD_RST for "
				"lcd reset control\n");
		return err;
	}
#endif
...
#if defined(CONFIG_DUAL_LCD)
	gpio_direction_output(GPIO_SLCD_RST, 1);
	msleep(10);

	gpio_set_value(GPIO_SLCD_RST, 0);
	msleep(10);

	gpio_set_value(GPIO_SLCD_RST, 1);
	msleep(10);

	gpio_free(GPIO_SLCD_RST);
...
static struct spi_board_info spi_board_info[] __initdata = {
	{
		.modalias	= "tl2796",
		.platform_data	= &aries_tft_panel_data,
...
#if !defined(CONFIG_MACH_S5PC110_LATONA) //TEMP for compile
...
static void set_shared_mic_bias(void)
{
	...
	gpio_set_value(GPIO_MICBIAS_EN_OLD, wm8994_mic_bias);
...
{
//yuezengzhen for W899GB
#if defined(CONFIG_MACH_S5PC110_ARIESCT) || defined(CONFIG_MACH_S5PC110_PRESTIGE)
	/* CAM_IO_EN - GPC0(0) */
	if(gpio_request(GPIO_CAM_IO_EN, "GPC0") < 0) {
	    pr_err("failed gpio_request(GPC0) for camera control\n");
	}
#else
	/* CAM_IO_EN - GPB(7) */
...
// kk0704.park FLASH SETTING
#if defined(CONFIG_MACH_S5PC110_PRESTIGE)
    if(gpio_request(GPIO_AP_FLASH_EN, "GPH3") < 0) {
        pr_err("failed gpio_request(GPH3) for camera control\n");
        return -EINVAL;
    }
	gpio_direction_output(GPIO_AP_FLASH_EN, 1);
	gpio_set_value(GPIO_AP_FLASH_EN, 1);
	gpio_free(GPIO_AP_FLASH_EN);
#endif
//kk0704.park FLASH SETTING
...
// kk0704.park FLASH SETTING
	...
	gpio_direction_output(GPIO_AP_FLASH_EN, 0);
	gpio_set_value(GPIO_AP_FLASH_EN, 0);
...
static struct s3c_platform_camera ce147 = {
	...
	.i2c_busnum	= 13,//yuezengzhen for W899GB
	.info		= &ce147_i2c_info,
	.pixelformat	= V4L2_PIX_FMT_UYVY,
	.srclk_name	= "xusbxti",
...
//yuezengzhen for W899GB
	/* CAM_VGA_nSTBY - GPB(0) */
#if defined(CONFIG_MACH_S5PC110_PRESTIGE) || defined(CONFIG_MACH_S5PC110_LATONA)
        err = gpio_request(GPIO_CAM_VGA_nSTBY, "GPC1");
        if (err) {
                pr_err("Failed to request GPC1 for camera control\n");
                return -EINVAL;
        }
        ...
        err = gpio_request(GPIO_CAM_VGA_nRST, "GPE1");
        if (err) {
                pr_err("Failed to request GPE1 for camera control\n");
                gpio_free(GPIO_CAM_VGA_nSTBY);
                return -EINVAL;
        }
        /* CAM_IO_EN - GPB(7) */
...
static int s5ka3dfx_power_off(void)
{
	...
	gpio_free(GPIO_CAM_IO_EN);
#else
	gpio_direction_output(GPIO_GPB7, 1); //(H/W timing problem)
	gpio_set_value(GPIO_GPB7, 0);
	gpio_free(GPIO_GPB7);
#endif
	gpio_free(GPIO_CAM_VGA_nRST);
...
extern u16 who_is_the_caller_of_fsa9480;
extern u16 who_is_the_caller_of_usbmenusel;
...
#endif
{
...
#if 0 // by kbj 
	int usb_settings_value , usb_settings_length;
	usb_settings_length = property_get("persist.service.usb.setting", usb_settings_value, 4); // default == 4 ASKON	
	printk("%s , usb_settings_value = %d \n" , __func__ , usb_settings_value);
	if ( usb_settings_length == 4 )
		askonstatus = 0xabcd;
#endif	
	...
	printk("%s , askonstatus = %d , who_is_the_caller_of_fsa9480=%d \n" , __func__ , askonstatus , who_is_the_caller_of_fsa9480);
	...
	if (gadget) {
		...
		{
			if (attached && who_is_the_caller_of_fsa9480 == CALL_FROM_USB_MENU_SEL)
			{
				usb_gadget_vbus_connect(gadget);
				who_is_the_caller_of_fsa9480 = CALL_FROM_USB_CABLE;
			}
			else if( !attached )
			{
	...
	if (charger_callbacks && charger_callbacks->set_cable)
		charger_callbacks->set_cable(charger_callbacks, set_cable_status);
	printk("%s set_cable_status=%d, attached=%d\n", __func__, set_cable_status, attached);
}
...
static struct sec_jack_zone sec_jack_zones[] = {
	...
	{
		/* 600 < adc <= 1800, 4 pole zone, default to 4pole if it
		 * stays in this range for 800ms (10ms delays, 80 samples)
		 */
		.adc_high = 1800,
		.delay_ms = 10,
	...
	{
		/* 1800 < adc <= 3250, unstable zone, default to 3pole if it
		 * stays in this range for 800ms (10ms delays, 80 samples)
		 */
		.adc_high = 3250,
		.delay_ms = 10,
	...
	{
		/* adc > 3250, unstable zone, default to 3pole if it stays
		 * in this range for two seconds (10ms delays, 200 samples)
		 */
		.adc_high = 0x7fffffff,
		.delay_ms = 10,
...
static void aries_power_off(void)
{
	...
	err = gpio_request(GPIO_QSC_ACTIVE, "GPIO_QSC_ACTIVE");
	WARN(err, "failed to request GPIO_QSC_ACTIVE"); // will never be freed

	err = gpio_request(GPIO_ESC_ACTIVE, "GPIO_ESC_ACTIVE");
	WARN(err, "failed to request GPIO_ESC_ACTIVE"); // will never be freed

	gpio_direction_input(GPIO_N_POWER);
	gpio_direction_input(GPIO_QSC_ACTIVE);
	gpio_direction_input(GPIO_ESC_ACTIVE);

	gpio_set_value(GPIO_QSC_PHONE_ON, 0);
	gpio_set_value(GPIO_ESC_PHONE_ON, 0);

	// confirm phone off
	while(1)
	{
		qsc_active = gpio_get_value(GPIO_QSC_ACTIVE);
		esc_active = gpio_get_value(GPIO_ESC_ACTIVE);
		if (!qsc_active && !esc_active)
		{
			printk(KERN_DEBUG "%s: already Phone Active Low\n",__func__);
			break;
		}
		else 
		{
			if (phone_wait_cnt > 10) {
				if (qsc_active) {
					printk(KERN_EMERG "%s: Try to Turn Phone Off by QSC_RST\n", __func__);
					gpio_set_value(GPIO_QSC_RESET_N, 0);
				}
				if (esc_active) {
					printk(KERN_EMERG "%s: Try to Turn Phone Off by ESC_RST\n", __func__);
					gpio_set_value(GPIO_ESC_RESET_N, 0);
				}
			}

			printk(KERN_DEBUG "%s: Wait QSC_ACTIVE(%d), ESC_ACTIVE(%d)\n",__func__,qsc_active,esc_active);
	...
	printk(KERN_DEBUG "%s: clear alarm poweron source\n", __func__);
	if (max8998_rtc_read_alarm0conf(&bootalarm_conf))
	{
		printk(KERN_DEBUG "%s: fail to read alarm0conf reg\n", __func__);
	}
...
//	set_pmic_gpio();
	...
	s3c_gpio_cfgpin(GPIO_KEY_BL_EN, S3C_GPIO_SFN(1));
	s3c_gpio_setpull(GPIO_KEY_BL_EN, S3C_GPIO_PULL_NONE);
...
static unsigned int jupiter_sleep_gpio_table[][3] = 
{
	/* off gpio part */
    // GPA0 -------------------------------------------------
	{S5PV210_GPA0(0), S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_DOWN},
	{S5PV210_GPA0(1), S3C_GPIO_SLP_OUT0,  S3C_GPIO_PULL_NONE},
	{S5PV210_GPA0(2), S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_DOWN},
	...
	{S5PV210_GPA0(7), S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_NONE},
    ...
    // GPB  -------------------------------------------------
	{S5PV210_GPB(0),  S3C_GPIO_SLP_OUT1,  S3C_GPIO_PULL_NONE},
	{S5PV210_GPB(1),  S3C_GPIO_SLP_OUT0,  S3C_GPIO_PULL_NONE},
	{S5PV210_GPB(2),  S3C_GPIO_SLP_OUT1,  S3C_GPIO_PULL_NONE},
	...
	{S5PV210_GPB(6),  S3C_GPIO_SLP_PREV,  S3C_GPIO_PULL_NONE},
	{S5PV210_GPB(7),  S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_DOWN},

    // GPC0 -------------------------------------------------
	{S5PV210_GPC0(0), S3C_GPIO_SLP_OUT0,  S3C_GPIO_PULL_NONE},
	{S5PV210_GPC0(1), S3C_GPIO_SLP_PREV,  S3C_GPIO_PULL_NONE},	//GPIO_CP_PCM_SEL
	{S5PV210_GPC0(2), S3C_GPIO_SLP_PREV,  S3C_GPIO_PULL_NONE},
	...
	{S5PV210_GPC1(1), S3C_GPIO_SLP_OUT0,  S3C_GPIO_PULL_NONE},
	{S5PV210_GPC1(2), S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_DOWN},
	{S5PV210_GPC1(3), S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_DOWN},
	{S5PV210_GPC1(4), S3C_GPIO_SLP_PREV,  S3C_GPIO_PULL_NONE},	//GPIO_MICBIAS_EN

    // GPD0 -------------------------------------------------
	{S5PV210_GPD0(0), S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_UP},
	{S5PV210_GPD0(1), S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_DOWN},
	{S5PV210_GPD0(2), S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_DOWN},
	{S5PV210_GPD0(3), S3C_GPIO_SLP_OUT1,  S3C_GPIO_PULL_NONE},
	...
	{S5PV210_GPF3(5), S3C_GPIO_SLP_PREV,  S3C_GPIO_PULL_NONE},
	...
	{S5PV210_GPG1(1), S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_NONE},
	...
	{S5PV210_GPG3(6), S3C_GPIO_SLP_OUT0, S3C_GPIO_PULL_NONE},
	...
	{S5PV210_GPI(4),  S3C_GPIO_SLP_OUT0,  S3C_GPIO_PULL_NONE},
	...
	{S5PV210_GPJ1(2), S3C_GPIO_SLP_PREV,  S3C_GPIO_PULL_NONE}, //GPIO_MIC_SEL
	...
	{S5PV210_GPJ3(7), S3C_GPIO_SLP_OUT0, S3C_GPIO_PULL_NONE},

    // GPJ4 -------------------------------------------------
	{S5PV210_GPJ4(0), S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_UP},
	{S5PV210_GPJ4(1), S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_UP},
	{S5PV210_GPJ4(2), S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_UP},
	{S5PV210_GPJ4(3), S3C_GPIO_SLP_OUT1,  S3C_GPIO_PULL_NONE},
	{S5PV210_GPJ4(4), S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_UP},
    ...
    // MP02 -------------------------------------------------
	{S5PV210_MP02(0), S3C_GPIO_SLP_OUT1,  S3C_GPIO_PULL_NONE},
    ...
    // MP04 -------------------------------------------------
	{S5PV210_MP04(0), S3C_GPIO_SLP_PREV,  S3C_GPIO_PULL_NONE},	//GPIO_MICBIAS_EN_OLD
	...
	{S5PV210_MP05(6), S3C_GPIO_SLP_OUT0,  S3C_GPIO_PULL_NONE},
...
#elif defined (CONFIG_MACH_S5PC110_GALAXY) || defined (CONFIG_MACH_S5PC110_LATONA) //elif of CONFIG_MACH_S5PC110_PRESTIGE
static unsigned int jupiter_sleep_gpio_table[][3] = 
{
	/* off gpio part */
    // GPA0 -------------------------------------------------
    ...
    // GPC0 -------------------------------------------------
	{S5PV210_GPC0(0), S3C_GPIO_SLP_OUT0,  S3C_GPIO_PULL_NONE},
	{S5PV210_GPC0(1), S3C_GPIO_SLP_PREV,  S3C_GPIO_PULL_NONE},
	{S5PV210_GPC0(2), S3C_GPIO_SLP_PREV,  S3C_GPIO_PULL_NONE},
	{S5PV210_GPC0(3), S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_DOWN},
	{S5PV210_GPC0(4), S3C_GPIO_SLP_OUT0,  S3C_GPIO_PULL_NONE},
	...
	{S5PV210_GPD1(2), S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_NONE},
	{S5PV210_GPD1(3), S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_NONE},
	{S5PV210_GPD1(4), S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_NONE}, //S3C_GPIO_PULL_DOWN,  to always turn on TSP 
	{S5PV210_GPD1(5), S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_NONE}, //S3C_GPIO_PULL_DOWN,  to always turn on TSP 
	...
	{S5PV210_GPJ0(3), S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_NONE},
	{S5PV210_GPJ0(4), S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_NONE},
	{S5PV210_GPJ0(5), S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_NONE}, //S3C_GPIO_PULL_DOWN,  to always turn on TSP 
	{S5PV210_GPJ0(6), S3C_GPIO_SLP_OUT0,  S3C_GPIO_PULL_NONE},
	{S5PV210_GPJ0(7), S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_NONE},
	...
	{S5PV210_GPJ1(0), S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_DOWN},
	{S5PV210_GPJ1(1), S3C_GPIO_SLP_OUT0,  S3C_GPIO_PULL_NONE},
	{S5PV210_GPJ1(2), S3C_GPIO_SLP_PREV,  S3C_GPIO_PULL_NONE},
	{S5PV210_GPJ1(3), S3C_GPIO_SLP_OUT0,  S3C_GPIO_PULL_NONE}, // to always turn on TSP 
	{S5PV210_GPJ1(4), S3C_GPIO_SLP_PREV,  S3C_GPIO_PULL_NONE},
	{S5PV210_GPJ1(5), S3C_GPIO_SLP_OUT0,  S3C_GPIO_PULL_NONE},
	...
	{S5PV210_MP01(0), S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_DOWN},
	{S5PV210_MP01(1), S3C_GPIO_SLP_OUT0,  S3C_GPIO_PULL_NONE},
	{S5PV210_MP01(2), S3C_GPIO_SLP_PREV,  S3C_GPIO_PULL_NONE},
	{S5PV210_MP01(3), S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_DOWN},
	{S5PV210_MP01(4), S3C_GPIO_SLP_OUT1,  S3C_GPIO_PULL_NONE},
	{S5PV210_MP01(5), S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_DOWN},
	{S5PV210_MP01(6), S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_DOWN},
...
void s3c_config_sleep_gpio_table(int array_size, unsigned int (*gpio_table)[3])
{
	...
	printk(KERN_DEBUG "SLPGPIO : CODEC_LDO_EN(%d) MICBIAS_EN(%d) GPIO_MIC_SEL(%d)\n",
		gpio_get_value(GPIO_CODEC_LDO_EN),gpio_get_value(GPIO_MICBIAS_EN),gpio_get_value(GPIO_MIC_SEL));
...
void config_sleep_gpio(void)
{
...
#if 0
	s3c_gpio_cfgpin(S5PV210_GPH1(2), S3C_GPIO_OUTPUT);//S3C_GPIO_INPUT
	s3c_gpio_setpull(S5PV210_GPH1(2), S3C_GPIO_PULL_NONE);//S3C_GPIO_PULL_DOWN
	...
	//s3c_gpio_setpin(S5PV210_GPH2(3), 0);
	...
	s3c_gpio_setpull(S5PV210_GPH3(1), S3C_GPIO_PULL_DOWN);
...
#elif defined (CONFIG_MACH_S5PC110_GALAXY) || defined (CONFIG_MACH_S5PC110_LATONA)
#if 1
	s3c_gpio_cfgpin(S5PV210_GPH2(0), S3C_GPIO_INPUT);
	s3c_gpio_setpull(S5PV210_GPH2(0), S3C_GPIO_PULL_DOWN);
	//s3c_gpio_setpin(S5PV210_GPH2(0), 0);

#else  // do not delete, it could be used.
	// setting the alive mode registers
...
#else  // defined (CONFIG_MACH_S5PC110_GALAXY) || defined (CONFIG_MACH_S5PC110_LATONA)
...
#endif  // defined (CONFIG_MACH_S5PC110_GALAXY) || defined (CONFIG_MACH_S5PC110_LATONA)
}
...
static struct platform_device *aries_devices[] __initdata = {
	...
	&s3c_device_i2c13, /* CE147 *///yuezengzhen for W899GB
#if !defined(CONFIG_MACH_S5PC110_LATONA) //TEMP for compile
...
#if defined(CONFIG_MACH_S5PC110_PRESTIGE) // pje_CI16

/*
 * W899 
 * Check Upper board connected or not.
 */
extern int hw_upper_board_check_touchkey(void);

int	s5pc110_upper_board = 1; // Default : Exist

void _hw_upper_board_check(void)
{
	/* 
	 * Read MOLED, 
	 * if High, Upper board connected,
	 * or Low,  Not connected.
	 */

	s5pc110_upper_board = 0;

	s3c_gpio_cfgpin(GPIO_OLED_DET, S3C_GPIO_INPUT);
	s3c_gpio_setpull(GPIO_OLED_DET, S3C_GPIO_PULL_DOWN); 

	s3c_gpio_cfgpin(GPIO_VIBTONE_PWM, S3C_GPIO_INPUT);
	s3c_gpio_setpull(GPIO_VIBTONE_PWM, S3C_GPIO_PULL_DOWN); 

	if (gpio_get_value(GPIO_OLED_DET))	{
		s5pc110_upper_board = 1;
	}
	printk("Upper Board is 0x%x\n", s5pc110_upper_board);
}

int hw_upper_board_check(void)
{
	if (gpio_get_value(GPIO_OLED_DET))	{
		s5pc110_upper_board = 1;
	} else	{
	...
	printk("s5pc110_upper_board:%X \n", s5pc110_upper_board);
//	printk("hw_upper_board_check_touchkey():%X \n", hw_upper_board_check_touchkey());

	return s5pc110_upper_board || hw_upper_board_check_touchkey();
}
EXPORT_SYMBOL(hw_upper_board_check);
...
static void __init fsa9480_gpio_init(void)
{
	...
	s3c_gpio_cfgpin(GPIO_CP_UART_SEL, S3C_GPIO_OUTPUT);
	s3c_gpio_setpull( GPIO_CP_UART_SEL, S3C_GPIO_PULL_NONE);
	
	s3c_gpio_cfgpin(GPIO_CP_USB_SEL, S3C_GPIO_OUTPUT);
	s3c_gpio_setpull( GPIO_CP_USB_SEL, S3C_GPIO_PULL_NONE); 

        //CTC_W899_huazai.liu
	s3c_gpio_cfgpin(S5PV210_GPJ1(0), S3C_GPIO_OUTPUT);
...
#else
	gpio_request(GPIO_MICBIAS_EN, "micbias_enable");
	gpio_request(GPIO_MICBIAS_EN_OLD, "old_micbias_enable");
#endif

/* CHN soon9.lee -start */

	//PCM SEL SETTING
	if (gpio_is_valid(GPIO_CP_PCM_SEL))
	{
		if (gpio_request(GPIO_CP_PCM_SEL, "CP_PCM_SEL"))
			printk("Failed to request CP_PCM_SEL! \n");
		
		gpio_direction_output(GPIO_CP_PCM_SEL, 1);
	}

	s3c_gpio_setpull(GPIO_CP_PCM_SEL, S3C_GPIO_PULL_UP);
	
	// For preserving output of codec related pins.
	s3c_gpio_slp_cfgpin(GPIO_CP_PCM_SEL, S3C_GPIO_SLP_PREV);
	//s3c_gpio_slp_setpull_updown(GPIO_CP_PCM_SEL, S3C_GPIO_PULL_NONE);

	if (gpio_is_valid(GPIO_MIC_SEL))
	{
		if (gpio_request(GPIO_MIC_SEL, "MIC_SEL"))
			printk("Failed to request MIC_SEL! \n");
	...
	// For preserving output of codec related pins.
	s3c_gpio_slp_cfgpin(GPIO_MIC_SEL, S3C_GPIO_SLP_PREV);	
...
#if defined(CONFIG_MACH_S5PC110_ARIESCT)
...
#if 1 //defined(CONFIG_MACH_S5PC110_ARIESCT) || defined(CONFIG_MACH_S5PC110_PRESTIGE)
	gpio = S5PV210_GPG1(3);                     /* XMMC3DATA_3 */
#else
...
static void __init aries_machine_init(void)
{
	...
	HWREV = 1;//gpio_get_value(GPIO_HWREV_MODE0);

	/* I don't want to change HWREV variable at other files, so I just make like below */
	if(HWREV)
		HWREV = 7;
#else
	s3c_gpio_cfgpin(GPIO_HWREV_MODE0, S3C_GPIO_INPUT);
	...
	printk("HWREV is 0x%x\n", HWREV);
		...
		printk("W899_HWREV is 0x%x\n", W899_HWREV);
		HWREV = 0x0A;
		printk("HWREV is fixed to 0x%x\n", HWREV);
#endif
	...
	/* nfc sensor */
#if !defined(CONFIG_MACH_S5PC110_PRESTIGE) && !defined(CONFIG_MACH_S5PC110_ARIESCT)
...
#if defined(CONFIG_MACH_S5PC110_PRESTIGE) // pje_CI16
	/* Check if uppoer board connected or not */
	if (W899_HWREV >= 0x09) {
		_hw_upper_board_check();
	} else	{
		s5pc110_upper_board = 1;
	}
#endif
