...
static ssize_t uart_sel_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	int uart_cp_sel = secsw->switch_sel & CP_UART_SEL_MASK;
	
//	printk(KERN_ERR "%s UART\n", uart_sel ? "PDA" : uart_cp_sel ? "ESC" : "QSC");
				 ...
				 const char *buf, size_t size)
{
	...
	if (strncmp(buf, "PDA", 3) == 0 || strncmp(buf, "pda", 3) == 0) {
		...
		printk(KERN_ERR "[UART Switch] Path : PDA\n");
	}

	if (strncmp(buf, "QSC", 3) == 0 || strncmp(buf, "qsc", 3) == 0) {
		gpio_set_value(GPIO_CP_UART_SEL, 0);
		gpio_set_value(GPIO_UART_SEL, 0);
		secsw->switch_sel &= ~UART_SEL_MASK;
		secsw->switch_sel &= ~CP_UART_SEL_MASK;
		printk(KERN_ERR "[UART Switch] Path : QSC\n");
	}

	if (strncmp(buf, "ESC", 3) == 0 || strncmp(buf, "esc", 3) == 0) {
		gpio_set_value(GPIO_CP_UART_SEL, 1);
		gpio_set_value(GPIO_UART_SEL, 0);
		secsw->switch_sel &= ~UART_SEL_MASK;
		secsw->switch_sel |= CP_UART_SEL_MASK;
		printk(KERN_ERR "[UART Switch] Path : ESC\n");
	}
...
static ssize_t usb_sel_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	int usb_cp_path = secsw->switch_sel & CP_USB_SEL_MASK;
//	printk(KERN_ERR "%s USB\n", usb_path ? "PDA" : usb_cp_path ? "ESC" : "QSC");
				...
				const char *buf, size_t size)
{
	...
	if (strncmp(buf, "QSC", 3) == 0 || strncmp(buf, "qsc", 3) == 0) {
		gpio_set_value(GPIO_CP_USB_SEL, 0);
		usb_switch_mode(secsw, SWITCH_MODEM);
		secsw->switch_sel &= ~USB_SEL_MASK;
		secsw->switch_sel &= ~CP_USB_SEL_MASK;
	}

	if (strncmp(buf, "ESC", 3) == 0 || strncmp(buf, "esc", 3) == 0) {
		gpio_set_value(GPIO_CP_USB_SEL, 1);
		usb_switch_mode(secsw, SWITCH_MODEM);
		secsw->switch_sel &= ~USB_SEL_MASK;
		secsw->switch_sel |= CP_USB_SEL_MASK;
	}
...
static void sec_switch_init_work(struct work_struct *work)
{
	...
	printk(KERN_ERR "%s : initial sec switch value = 0x%X\n", __func__, secsw->switch_sel);
	...
	/* init UART/USB path */
	if (usb_sel) {
		usb_switch_mode(secsw, SWITCH_PDA);		// PDA
	}
	else {
		if (secsw->switch_sel & CP_USB_SEL_MASK)
			gpio_set_value(GPIO_CP_USB_SEL, 1);	// ESC
		else
			gpio_set_value(GPIO_CP_USB_SEL, 0);	// QSC
		
		usb_switch_mode(secsw, SWITCH_MODEM);
	}
	
	if (uart_sel) {
		gpio_set_value(GPIO_UART_SEL, 1);			// PDA
	}		
	else {
		if (secsw->switch_sel & CP_UART_SEL_MASK)
			gpio_set_value(GPIO_CP_UART_SEL, 1);	// ESC
		else
			gpio_set_value(GPIO_CP_UART_SEL, 0);	// QSC
