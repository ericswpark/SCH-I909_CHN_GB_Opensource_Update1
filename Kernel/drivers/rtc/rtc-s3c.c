...
static int s3c_rtc_getbootalarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	struct rtc_time *alm_tm = &alrm->time;

    if (max8998_rtc_read_alarm0(alm_tm))
    {
        pr_err("%s : fail max8998_rtc_read_alarm0\n",__func__);
        return 1;
    }

    if (max8998_rtc_read_alarm0conf(&alrm->enabled))
    {
        pr_err("%s : fail max8998_rtc_read_alarm0conf\n",__func__);
    }
...
static int s3c_rtc_setbootalarm(struct device *dev, struct rtc_wkalrm *alrm)
{
    struct rtc_time *tm = &alrm->time;

    pr_debug("%s : (%d) %02d.%02d.%02d %02d/%02d/%02d\n",__func__,alrm->enabled,
        tm->tm_year, tm->tm_mon, tm->tm_mday, tm->tm_hour, tm->tm_min, tm->tm_sec);

    if (max8998_rtc_set_alarm0(tm))
    {
        pr_err("%s : fail max8998_rtc_set_alarm0\n",__func__);
        return 1;
    }

    if (max8998_rtc_set_alarm0conf(alrm->enabled))
    {
        pr_err("%s : fail max8998_rtc_set_alarm0conf\n",__func__);
        return 1;
    }
...
static const struct rtc_class_ops s3c_rtcops = {
	...
	.read_bootalarm = s3c_rtc_getbootalarm,
	.set_bootalarm  = s3c_rtc_setbootalarm,
