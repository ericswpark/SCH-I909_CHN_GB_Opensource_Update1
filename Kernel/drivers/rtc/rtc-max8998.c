...
#define MAX8998_RTC_TIME_ADDR		0x0
#define MAX8998_RTC_ALARM0_ADDR		0x8
#define MAX8998_RTC_ALARM1_ADDR		0x10
#define MAX8998_RTC_ALRM0CONF_REG   0x18
#define MAX8998_RTC_ALRM1CONF_REG   0x19
#define MAX8998_RTC_WTSR_SMPL_REG   0x1B
...
static int max8998_rtc_i2c_read_regs(struct i2c_client *client, u8 type, struct rtc_time *tm)
{
    int ret;
    u8 regs[MAX8998_RTC_LEN];

    ret = max8998_rtc_i2c_read(client, type, regs, MAX8998_RTC_LEN);
    if (ret < 0)
        return ret;
...
static int max8998_rtc_i2c_set_regs(struct i2c_client *client, u8 type, struct rtc_time *tm)
{
    int ret;
	...
	ret = max8998_rtc_i2c_write(client, type, regs, MAX8998_RTC_LEN +1);
...
{
	int ret;
	ret = max8998_rtc_i2c_read_regs(max8998_rtc_i2c_client, MAX8998_RTC_TIME_ADDR, tm);
...
int max8998_rtc_set_time(struct rtc_time *tm)
{
	max8998_dbg("%s  %02d.%02d.%02d %02d/%02d/%02d\n",__func__,
		 tm->tm_year, tm->tm_mon, tm->tm_mday, tm->tm_hour, tm->tm_min, tm->tm_sec);
	return max8998_rtc_i2c_set_regs(max8998_rtc_i2c_client, MAX8998_RTC_TIME_ADDR, tm);
}

int max8998_rtc_read_alarm0(struct rtc_time *tm)
{
	int ret;
	ret = max8998_rtc_i2c_read_regs(max8998_rtc_i2c_client, MAX8998_RTC_ALARM0_ADDR, tm);
	max8998_dbg("read %s time %02d.%02d.%02d %02d/%02d/%02d\n",__func__,
		 tm->tm_year, tm->tm_mon, tm->tm_mday, tm->tm_hour, tm->tm_min, tm->tm_sec);
	return ret;
}

int max8998_rtc_set_alarm0(struct rtc_time *tm)
{
	max8998_dbg("%s  %02d.%02d.%02d %02d/%02d/%02d\n",__func__,
		 tm->tm_year, tm->tm_mon, tm->tm_mday, tm->tm_hour, tm->tm_min, tm->tm_sec);
	return max8998_rtc_i2c_set_regs(max8998_rtc_i2c_client, MAX8998_RTC_ALARM0_ADDR, tm);
}

int max8998_rtc_read_alarm0conf(int *onoff)
{
    int ret;
    u8 regs;
    ret = max8998_rtc_i2c_read(max8998_rtc_i2c_client, MAX8998_RTC_ALRM0CONF_REG, &regs, 1);
    *onoff = (regs == 0x77);
    max8998_dbg("%s  reg = %x (%d)\n",__func__,regs,*onoff);
    return ret;
}

int max8998_rtc_set_alarm0conf(int onoff)
{
    u8 regs[2];
    regs[1] = onoff ? 0x77 : 0x0;
    max8998_dbg("%s  reg = %x\n",__func__,regs[1]);
    return max8998_rtc_i2c_write(max8998_rtc_i2c_client, MAX8998_RTC_ALRM0CONF_REG, regs, 2);
}
