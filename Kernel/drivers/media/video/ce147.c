...
#define CMD_SET_FLASH_AMOUNT_DISTANCE	0x07 //yuezengzhen for W899GB
...
#define CMD_INFO_SCENE		0xA5 //yuezengzhen for W899GB
#define	CMD_SET_FLASH			0xB2
#define CMD_SET_FLASH_BRIGHTNESS		0xB3 //yuezengzhen for W899GB
...
//MM-CAM kk0704.park DG07 Change EXIF sw version
#define CE147_SW_VERSION	"/system/build.prop" //yuezengzhen for W899GB
...
enum ce147_frame_size {
	...
	CE147_CAPTURE_QCIF, /* 176 x 144 */	
	CE147_CAPTURE_QVGA, /* 320 x 240 */	
	CE147_CAPTURE_WQVGA, /* 400 x 240 */	
	CE147_CAPTURE_VGA,	/* 640 x 480 */
	CE147_CAPTURE_D1, /* 720 x 480 */	
	CE147_CAPTURE_WVGA,	/* 800 x 480 */
	CE147_CAPTURE_XGA, /* 1024 x 768 */
	CE147_CAPTURE_720P, /* 1280 x 720 */	
	CE147_CAPTURE_960P, /* 1280 x 960 */	
...
static struct ce147_enum_framesize ce147_framesize_list[] = {
	...
	{ CE147_OPRMODE_IMAGE, CE147_CAPTURE_QCIF,	176,  144 },
	{ CE147_OPRMODE_IMAGE, CE147_CAPTURE_QVGA,	320,  240 },
	{ CE147_OPRMODE_IMAGE, CE147_CAPTURE_WQVGA,	400,  240 },
	{ CE147_OPRMODE_IMAGE, CE147_CAPTURE_VGA,	 640,  480 },
	{ CE147_OPRMODE_IMAGE, CE147_CAPTURE_D1,	720,  480 },	
	{ CE147_OPRMODE_IMAGE, CE147_CAPTURE_WVGA,	 800,  480 },
	{ CE147_OPRMODE_IMAGE, CE147_CAPTURE_XGA,	1024,  768 },
	{ CE147_OPRMODE_IMAGE, CE147_CAPTURE_720P,	1280,  720 },	
	{ CE147_OPRMODE_IMAGE, CE147_CAPTURE_960P,	1280,  960 },	
...
struct ce147_state {
	...
	struct tm2 *exifTimeInfo; //yuezengzhen for W899GB
...
static int condition = 0;//yuezengzhen for W899GB
...
{
	int retry_count = 3;//yuezengzhen for W899GB
		...
		unsigned char *r_data, unsigned int r_len)
{
	...
	retry_count = 3;//yuezengzhen for W899GB
	while(retry_count--){
		ret  = i2c_transfer(client->adapter, &msg, 1);
		if(ret == 1)
			break;
...
//yuezengzhen for W899GB
static void ce147_change_sw_version(char* strDate, char* swVersion)
{
	unsigned short year, month, day;

	year =  (strDate[0]  - '0') * 1000;
	year += (strDate[1]  - '0') * 100;
	year += (strDate[2]  - '0') * 10;
	year += (strDate[3] - '0');

	month = (strDate[4] - '0') * 10;
	month += (strDate[5] - '0');

	swVersion[0] = (char)('A' + ((year+21)%26));
	swVersion[1] = (char)('A' + month - 1);
	swVersion[2] = strDate[6];
	swVersion[3] = strDate[7];
	swVersion[4] = '\0';
}

//MM-CAM kk0704.park DG07 Change EXIF sw version //CTC_W899_seungjoo.yi
static int ce147_read_sw_version(const char *path, char *swVersion, int *swVersionSize)
{
	char*       buffer = NULL;
	...
	struct file *filep = NULL;
	mm_segment_t old_fs;
	const char compareVersionName[6] = "date=";
	char strDate[9];
	int i, count=0;
	

	//printk("ce147_read_sw_version is called...\n");
	...
	if (filep && (filep!= 0xfffffffe))
	{
		...
		startPosition = strstr(buffer, compareVersionName) + 5;
		...
		if (startPosition != NULL)	{
			for(i=0; ; i++)	{
				if(startPosition[i]== '.')	{
					...
					break;
				}else if(startPosition[i] < '0' || startPosition[i] > '9')	{
					continue;
				}
				strDate[count] = startPosition[i];
				count++;
			}
			for(i=0; ; i++)	{
				if(startPosition[i]== '.')	{
			...
			ce147_change_sw_version(strDate, swVersion);
		} else	{
			swVersion[0] = 'D';
			swVersion[1] = 'G';
			swVersion[2] = '0';
...
static int ce147_set_capture_size(struct v4l2_subdev *sd)
{
	...
	//yuezengzhen for W899GB
	switch(index){
	case CE147_CAPTURE_QCIF: /* 176x144 */
		ce147_regbuf_capture_size[0] = 0x1E;
		break;
	case CE147_CAPTURE_QVGA: /* 320x240 */
		ce147_regbuf_capture_size[0] = 0x03;
		break;
	case CE147_CAPTURE_WQVGA: /* 400x240 */
		ce147_regbuf_capture_size[0] = 0x12;
		break;
	case CE147_CAPTURE_VGA: /* 640x480 */
		ce147_regbuf_capture_size[0] = 0x04;
		break;
	case CE147_CAPTURE_D1: /* 720x480 */
		ce147_regbuf_capture_size[0] = 0x20;
		break;
	case CE147_CAPTURE_WVGA: /* 800x480 */
		ce147_regbuf_capture_size[0] = 0x13;
		break;
	case CE147_CAPTURE_XGA: /* 1024x768 */
		ce147_regbuf_capture_size[0] = 0x06;
		break;
	case CE147_CAPTURE_720P: /* 1280x720 */
		ce147_regbuf_capture_size[0] = 0x16;
		break;
	case CE147_CAPTURE_960P: /* 1280x960 */
		ce147_regbuf_capture_size[0] = 0x07;
		break;
...
static int ce147_set_capture_exif(struct v4l2_subdev *sd)
{
	...
	unsigned char ce147_software_version[130] = {0x00,};//yuezengzhen for W899GB
...
//yuezengzhen for W899GB	
	...
	unsigned char ce147_str_model[9] = "SCH-W899";
	unsigned char ce147_str_software_version[5] = "DG06";
#elif defined(CONFIG_MACH_S5PC110_GALAXY)
	unsigned char ce147_str_model[9] = "SCH-i909";
	unsigned char ce147_str_software_version[5] = "DG06";
#elif defined(CONFIG_MACH_S5PC110_LATONA)
	unsigned char ce147_str_model[9] = "SCH-i809";
	unsigned char ce147_str_software_version[5] = "DG06";
...
#endif
#if 1//yuezengzhen for W899GB
	struct timeval curr_time;
	struct rtc_time time;
#endif
	//MM-CAM kk0704.park DF16 :: inserted scene mode type
	unsigned char ce147_scene_type[1] = {0x00};

	//MM-CAM kk0704.park DG07 Change EXIF sw version
	char sw_version[10] = {0, };
	int sw_version_size = 0;

	err = ce147_read_sw_version(CE147_SW_VERSION, sw_version, &sw_version_size);
	if(err < 0)
	{
	...
	if (state->sa_status == SMART_AUTO_STATUS_LANDSCAPE)
		ce147_scene_type[0] = 0x01;
	else if (state->sa_status == SMART_AUTO_STATUS_PORTRAIT)
		ce147_scene_type[0] = 0x02;	
	else if (state->sa_status == SMART_AUTO_STATUS_NIGHT)
		ce147_scene_type[0] = 0x03;			
	else
		ce147_scene_type[0] = 0x00;	
	...
	//yuezengzhen for W899GB
	ce147_software_version[0] = 0x07;
	ce147_software_version[1] =0x05;
	//MM-CAM kk0704.park DG07 Change EXIF sw version
	memcpy(ce147_software_version+2, sw_version, sw_version_size);
	...
	gps_timestamp.tm_year += 2000;//yuezengzhen for W899GB
	...
	//yuezengzhen for W899GB
	err = ce147_i2c_write_multi(client, CMD_INFO_MODEL, ce147_software_version, ce147_reglen_model);
	if(err < 0){
		dev_err(&client->dev, "%s: failed: i2c_write for exif software version\n", __func__);
		return -EIO;
	}
...
//yuezengzhen for W899GB	
//MM-CAM kk0704.park DF16 :: inserted scene mode type
	err = ce147_i2c_write_multi(client, CMD_INFO_SCENE, ce147_scene_type , sizeof(ce147_scene_type));
	if(err < 0){
		dev_err(&client->dev, "%s: failed: i2c_write for scene\n", __func__);
		return -EIO;
	}
...
static int ce147_set_flash(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	static unsigned char ce147_buf_set_flash[2] = { 0x03, 0x00 };
	unsigned char buf_flash[2] = {0x01, 0x00};
	unsigned int ce147_len_set_flash = 2;
	unsigned char ce147_buf_set_flash_bright[4] = { 0x00, 0x00, 0x07, 0x02 };
	unsigned int ce147_len_set_flash_bright = 4;	
	unsigned char ce147_buf_set_flash_manual_on[3] = { 0x00, 0x01 };
	unsigned char ce147_buf_set_flash_manual_off[2] = { 0x00, 0x00 };
	unsigned int ce147_len_set_flash_manual = 2;
	unsigned char ce147_buf_stop_lens_movement[1] = { 0x00 };
	unsigned char ce147_buf_led_capt_spark_ctrl[2] = { 0x02, 0x00 };
	...
	case FLASH_MODE_ON:
		...
		case FLASH_MODE_MAIN:
			ce147_buf_led_capt_spark_ctrl[1] = state->af_status;
			err = ce147_i2c_write_multi(client, CMD_SET_FLASH_AMOUNT_DISTANCE, ce147_buf_led_capt_spark_ctrl, ce147_len_set_flash);
			if(err < 0){
				dev_err(&client->dev, "%s: failed: i2c_write for set_flash_amount_distance\n", __func__);
				return -EIO;
			}

			ce147_buf_set_flash[0] = 0x03;
			...
			err = ce147_i2c_write_multi(client, CMD_SET_FLASH, buf_flash, ce147_len_set_flash);
			if(err < 0){
				dev_err(&client->dev, "%s: failed: i2c_write for set_flash\n", __func__);
				return -EIO;
			}
...
//yuezengzhen for W899GB
		case FLASH_MODE_SUB:
			...
			buf_flash[1] = ce147_buf_set_flash[1];
			err = ce147_i2c_write_multi(client, CMD_SET_FLASH, buf_flash, ce147_len_set_flash);
			if(err < 0){
				dev_err(&client->dev, "%s: failed: i2c_write for set_flash\n", __func__);
				return -EIO;
		...
		case FLASH_MODE_TORCH_ON:
			//zhoujiang modify to let torch flash
			//if(ce147_buf_set_flash[1] == 0x01) {
				err = ce147_i2c_write_multi(client, CMD_SET_FLASH, buf_flash, ce147_len_set_flash);
				if(err < 0){
					dev_err(&client->dev, "%s: failed: i2c_write for set_flash\n", __func__);
					return -EIO;
				...
				err = ce147_i2c_write_multi(client, CMD_SET_FLASH_BRIGHTNESS, ce147_buf_set_flash_bright, ce147_len_set_flash_bright);
				if(err < 0){
					dev_err(&client->dev, "%s: failed: i2c_write for set_flash_brightness\n", __func__);
					return -EIO;
				}

				err = ce147_i2c_write_multi(client, CMD_SET_FLASH_MANUAL, ce147_buf_set_flash_manual_on, ce147_len_set_flash_manual);
				if(err < 0){
					dev_err(&client->dev, "%s: failed: i2c_write for set_flash_manual_on\n", __func__);
					return -EIO;
				}
			//}
		break;
		...
		case FLASH_MODE_TORCH_OFF:
			...
			err = ce147_i2c_write_multi(client, CMD_SET_FLASH_MANUAL, ce147_buf_set_flash_manual_off, ce147_len_set_flash_manual);
			if(err < 0){
				dev_err(&client->dev, "%s: failed: i2c_write for set_flash\n", __func__);
				return -EIO;
			}
...
//MM-CAM kk0704.park DF16 :: inserted scene mode type //yuezengzhen for W899GB
static int ce147_set_scene_mode(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
					...
					struct v4l2_control *ctrl)
{
	...
	//yuezengzhen for W899GB
	unsigned int ce147_len_set_focus_mode = 1; //CTC_W899_seungjoo.yi

	unsigned char ce147_buf_set_flash_amount[2] = { 0x02, 0x00 };
	unsigned int ce147_len_set_flash_amount = 2;

	unsigned char ce147_buf_set_fd_lock[1] = { 0x00 };	//kk0704.park DF09 :: release FD
	unsigned int ce147_len_set_fd_lock = 1;	//kk0704.park DF09 :: release FD
	switch (ctrl->value) {
	case FOCUS_MODE_MACRO:
	case FOCUS_MODE_MACRO_DEFAULT:
		ce147_buf_set_focus_mode[0] = 0x01;
		ce147_buf_set_flash_amount[1] = 0x01; //CTC_W899_seungjoo.yi
		break;
	...
	default:
		...
		ce147_buf_set_flash_amount[1] = 0x00;
		ce147_buf_set_fd_lock[0] = 0x00;	//kk0704.park DF09 :: release FD
		break;
	}
#if 0
...
#endif
		msleep(50); //sjlee_0107 (need to delay to stablize AF)
...
#ifdef CONFIG_MACH_S5PC110_PRESTIGE //sjlee_0107 (W899 only has a flash)
		//CTC_W899_seungjoo.yi
		err = ce147_i2c_write_multi(client, CMD_SET_FLASH_AMOUNT_DISTANCE, ce147_buf_set_flash_amount, ce147_len_set_flash_amount);
		if(err < 0){
			dev_err(&client->dev, "%s: failed: i2c_write for flash_amount\n", __func__);
			return -EIO;
		}
#endif
		err = ce147_i2c_write_multi(client, CMD_SET_FACE_LOCK, ce147_buf_set_fd_lock, ce147_len_set_fd_lock);	//kk0704.park DF09 :: release FD
		if(err < 0){
...
static void ce147_init_parameters(struct v4l2_subdev *sd)
{
	...
	state->fps = 30; /* Default value *///yuezengzhen for W899GB
...
static int ce147_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	* which can be used in image capture also */
	...
	case V4L2_CID_CAMERA_CHECK_DATALINE_STOP:
		...
		break;
//yuezengzhen for W899GB		
//MM-CAM kk0704.park DF16 :: inserted scene mode type //CTC_W899_seungjoo.yi
	case V4L2_CID_CAMERA_SCENE_MODE:		
		err = ce147_set_scene_mode(sd, ctrl);
		break;//MM-CAM end		
