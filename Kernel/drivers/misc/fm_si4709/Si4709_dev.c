...
int Si4709_dev_RDS_data_get(radio_data_t *data)
{
	...
	printk("Si4709_dev_RDS_data_get called");
	...
	if (Si4709_dev.valid == eFALSE)	{
		printk("Si4709_dev_RDS_data_get called when DS is invalid");
		ret = -1;
	} else {
#ifdef RDS_INTERRUPT_ON_ALWAYS

		printk("RDS_Buffer_Index_read = %d",RDS_Buffer_Index_read);

		/*If No New RDS Data is available return error*/
		if(RDS_Buffer_Index_read == RDS_Buffer_Index_write) {
			printk("No_New_RDS_Data_is_available");
			...
			if((ret = i2c_read(READCHAN)) < 0)
				...
				printk("curr_channel: %u, curr_rssi:%u",data->curr_channel,(u32)data->curr_rssi);
			}
			ret = -1;
		}
		else {
