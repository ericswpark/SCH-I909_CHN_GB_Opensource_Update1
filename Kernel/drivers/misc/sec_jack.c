...
void sendend_status_change(int status)
{
	if(status)	{
		input_report_key(hi->input_dev, KEY_MEDIA, 1);
		input_sync(hi->input_dev);
		switch_set_state(&switch_sendend, 1);
		send_end_pressed = 1;
		printk("[ JACK_DRIVER ] Button is pressed.\n");
	} else	{
		input_report_key(hi->input_dev, KEY_MEDIA, 0);
		input_sync(hi->input_dev);
		switch_set_state(&switch_sendend, 0);
		send_end_pressed = 0;
		printk("[ JACK_DRIVER ] Button is released.\n");
	}
}
...
static int sec_jack_probe(struct platform_device *pdev)
{
	...
	hi_temp = kzalloc(sizeof(struct sec_jack_info), GFP_KERNEL);
	if (hi_temp == NULL) {
		pr_err("%s : Failed to allocate memory.\n", __func__);
		ret = -ENOMEM;
		goto err_kzalloc;
	 ...
	 * it is unique relative to other gpio_event devices
	 */
	hi_temp->dev_id = pdev->id;

	hi_temp->input_dev = input_allocate_device();
	if (hi_temp->input_dev == NULL) {
		ret = -ENOMEM;
		pr_err("%s : Failed to allocate input device.\n", __func__);
		input_free_device(hi_temp->input_dev);
//		goto err_request_input_dev;
	}

	hi_temp->input_dev->name = "sec_jack";
	input_set_capability(hi_temp->input_dev, EV_KEY, KEY_MEDIA);
	ret = input_register_device(hi_temp->input_dev);
	if (ret) {
...
#endif /* !defined(CONFIG_MACH_S5PC110_PRESTIGE) */
