...
#define FSA9480_INTMASK1_VAL				0xE0

#define FSA9480_CONNECT_TO_AUDIO_L_AND_R	0x48
#define FSA9480_CONNECT_TO_V_AUDIO_L_AND_R	0x90
#define FSA9480_CONNECT_TO_VIDEO			0x15
#endif
...
extern void jack_status_change(int status);
extern void sendend_status_change(int status);
extern void vps_status_change(int status);
#endif /* CONFIG_MACH_S5PC110_PRESTIGE */

#if defined(CONFIG_MACH_S5PC110_PRESTIGE)
#define INT1_LKP		0x08
#define INT1_LKR		0x10
#endif /* CONFIG_MACH_S5PC110_PRESTIGE */
...
static int is_usb = 0; 
/* Warning: Please call once!!! */
int fsa9480_is_bootusb(void)
{
	return is_usb; 
}
EXPORT_SYMBOL(fsa9480_is_bootusb); 
#endif
...
#if defined(CONFIG_MACH_S5PC110_PRESTIGE)	 
		data = FSA9480_CONNECT_TO_V_AUDIO_L_AND_R;
#else
		data = SW_AUDIO;
#endif
		value &= ~CON_MANUAL_SW;
...
static void fsa9480_detect_dev(struct fsa9480_usbsw *usbsw)
{
	...
	if (val1 || val2) {
		...
		/* CHARGER */
			...
			//printk("Attached val1=0x%x, val2=0x%x, dev1=0x%x, dev2=0x%x\n", val1, val2, usbsw->dev1, usbsw->dev2);
		...
		/* Car Dock */
			...
			printk("FSA9480_ATTACHED (DEV_JIG_UART_ON) \n");
			set_tsp_for_ta_detect(1);
#if 0 // W899, for Automation Test
			if (pdata->cardock_cb)
...
#else
			if (pdata->usb_cb){
 // by kbj , if this value is set as zero , all of the next tries can be not recognized as ASKON connection
		...
		/* CHARGER */
			...
			//printk("Detached val1=0x%x, val2=0x%x, dev1=0x%x, dev2=0x%x\n", val1, val2, usbsw->dev1, usbsw->dev2);
		...
		/* Car Dock */
			...
			printk("FSA9480_DETACHED (DEV_JIG_UART_ON) \n");
...
#ifdef CONFIG_MACH_S5PC110_PRESTIGE
	/* mask interrupts (unmask attach/detach/LKR/KRP only) soon9.lee 20110623 */
	ret = i2c_smbus_write_word_data(client, FSA9480_REG_INT1_MASK, 0x1fe4);
	if (ret < 0)
		dev_err(&client->dev, "%s: err %d\n", __func__, ret);
#else
...
#ifdef CONFIG_MACH_S5PC110_PRESTIGE
	if(intr & INT1_LKP) {
		sendend_status_change(1);
	}
	else if(intr & INT1_LKR) {
		sendend_status_change(0);
	}
	else
#endif /* CONFIG_MACH_S5PC110_PRESTIGE */
	{
...
#if 0//def CONFIG_PM
/* we do not use fsa9485, so do not check at resume*/
