...
static int mmc_blk_issue_rq(struct mmc_queue *mq, struct request *req)
{
		...
		if (brq.cmd.error || brq.data.error || brq.stop.error) {
			if (brq.data.blocks > 1 && rq_data_dir(req) == READ) {
				...
				printk(KERN_WARNING "%s: retrying using single "
				       "block read retry %d\n", req->rq_disk->disk_name, mq->rx_retries);
				#if 0
...
#if defined (CONFIG_MACH_S5PC110_GALAXY) || defined (CONFIG_MACH_S5PC110_LATONA)
			if(brq.data.error)	{
				printk(KERN_ERR "skip retry : error %d \n", brq.data.error);
			}
			else
#endif				
			if (rq_data_dir(req) == READ) {
 ...
 cmd_err:
	 ...
	 */
	mmc_assume_damaged = 1;
	printk(KERN_ERR "mmc cmd_err %d \n");
	if (mmc_card_sd(card)) {
		u32 blocks;
