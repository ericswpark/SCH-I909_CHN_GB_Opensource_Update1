...
#define NOISE_FILTER	//for AT&T TA issue
#define GUMI_NOISE	//factory noise test
...
unsigned char test_node[TEST_POINT_NUM] = {	15,				22,				116,		197,		204}; 	   
#else
...
#if defined(CONFIG_MACH_S5PC110_PRESTIGE)
static int qt602240_in_sleep = 0;
#endif
...
extern int  hw_upper_board_check(void);
static DEFINE_MUTEX(qt602240_use);
#endif
...
#if 0 // temp
static void do_quantum_unlock_timer(struct work_struct *work);
static DECLARE_DELAYED_WORK(quantum_unlock_work, do_quantum_unlock_timer);
#endif
#define CPUFREQ_UNLOCK_TIME msecs_to_jiffies(2000)
...
#ifdef NOISE_FILTER // 20111116 add "if 1"
...
void qt_Acquisition_Config_Init(void)
{
    ...
    acquisition_config.atchcalsthr = 40;//23;	// 35;
...
void qt_Multitouchscreen_Init(void)
{
...
#if defined(CONFIG_MACH_S5PC110_PRESTIGE)
    if (qt602240->working_tsp == QT602240_MAIN_TSP)
...
#if defined(CONFIG_MACH_S5PC110_PRESTIGE)
    touchscreen_config.movhystn = 1;///1;
    ...
    touchscreen_config.xloclip = 20;//0;
    touchscreen_config.xhiclip = 20;//0;
    touchscreen_config.yloclip = 30;//0;
    touchscreen_config.yhiclip = 30;//0;

    touchscreen_config.xedgectrl = 136;//143;
    touchscreen_config.xedgedist = 30;//56;//40;
    touchscreen_config.yedgectrl = 136;//143;
    touchscreen_config.yedgedist = 96;//80;
...
void qt_Grip_Face_Suppression_Config_Init(void)
{
    ...
    gripfacesuppression_config.shpthr2 = 15; //35;
...
void qt_Noise_Suppression_Config_Init(void)
{
	...
	if (info_block->info_id.family_id == 0x80) 
	{
		...
		noise_suppression_config.actvgcafvalid = 0;//3;
		...
		noise_suppression_config.idlegcafvalid = 0;//3;
		...
		noise_freq_table.freq_for_fherr4[0] = 24;
		noise_freq_table.freq_for_fherr4[1] = 35;
		noise_freq_table.freq_for_fherr4[2] = 42;
		noise_freq_table.freq_for_fherr4[3] = 50;
		noise_freq_table.freq_for_fherr4[4] = 57;
...
void qt_Noise_Suppression_Config_Init(void)
{
    ...
    noise_suppression_config.freq[0] = 5;//20;	//29; 	//5;	// 6;//10;
    noise_suppression_config.freq[1] = 15;//24;	//34;	//15;	// 11;//15;
    noise_suppression_config.freq[2] = 25;//30;	//39;	//25;	//16;//20;
    noise_suppression_config.freq[3] = 35;//40;	//49;	//35;	// 19;//25;
    noise_suppression_config.freq[4] = 45;//48;	//58;	//45;	// 21;//30;
...
uint8_t type_to_report_id(uint8_t object_type, uint8_t instance)
{
    ...
    {
        if((report_id_map[report_id].object_type == object_type) && (report_id_map[report_id].instance == instance))
        {
            report_id_found = 1;
        }
        else
...
#if 1
#if defined(CONFIG_MACH_S5PC110_PRESTIGE)
	if (init_touch_driver( qt602240->client->addr ) == DRIVER_SETUP_OK)
#else
...
#else
#if defined(CONFIG_MACH_S5PC110_PRESTIGE)
	if (init_touch_driver( qt602240->client->addr , (void *) &message_handler) == DRIVER_SETUP_OK)
#else
...
//		printk("Report ID to Object type / instance mapping:\n\r");
...
//			printk("[TSP] Report ID : %d, Object Type : T%d, Instance : %d\n\r",report_id ,object_type,instance);
		}
		...
		{
			//printk("Backed up the config to non-volatile memory!\n\r");
		}
...
int set_tsp_for_ta_detect(int state)
{
	...
	if(qt602240_in_sleep)
	{
		printk("qt602240_in_sleep\n");
		return 0;
	}
	...
	if(state)
		...
		noise_suppression_config.noisethr = 20;//13;	//20;		   
		...
		gumi_err_setting(0);	
#endif
		   touchscreen_config.tchthr = 40;//26;		//40;
		   noise_suppression_config.noisethr = 30;//20;	//30;		   
...
static int freq_hop_err_setting(int state)
{
	...
	printk("[TSP] t9_blen_for_fherr_cnt(%d) state(%d)\n",noise_freq_table.t9_blen_for_fherr_cnt,state);
}
...
/*extern */int is_tsp_set_ta=1;
/*extern */charging_device_type curent_device_type=0;
			...
			if( quantum_msg[0] == 14 )
			{
				...
				{ 
					...
					for ( i= 0; i<MAX_USING_FINGER_NUM; ++i )
					{
						...
						if(i == 0){
							//schedule_delayed_work(&quantum_unlock_work, CPUFREQ_UNLOCK_TIME);
							//s5pv210_unlock_dvfs_high_level(DVFS_LOCK_TOKEN_4);							
							//set_dvfs_perf_level();
							touch_state_val=0;
							}
...
//					cal_check_flag = 2u;
				...
				{ /* noise suppression */
					printk("[TSP] noise suppression !\n");
					if ((quantum_msg[1]&0x08) == 0x08) 
					{
#ifdef GUMI_NOISE
						if(curent_device_type==PM_CHARGER_TA || curent_device_type==PM_CHARGER_USB_INSERT){
								// at & t
						noise_freq_table.fherr_count++;
						if (noise_freq_table.fherr_count > 12)
						{
							printk("[TSP] noise fherr_count 13 !\n");
						...
						{
							printk("[TSP] freq changed. noise level too high. %d\n", noise_freq_table.fherr_count/3);
							freq_hop_err_setting(noise_freq_table.fherr_count/3);
						}

					}
						else{
							printk("[TSP] gumi noise !\n");							
							gumi_err_setting(1);	// gumi noise					
				}
#endif
					}
				}
...
//				printk(KERN_DEBUG "[TSP]### F [%d] U - num is (%d)  status=0x%02x\n", id, --qt_touch_num_state[id], quantum_msg[1]);
			...
			#if USE_PERF_LEVEL_TS
				if(id == 0){
					//s5pv210_lock_dvfs_high_level(DVFS_LOCK_TOKEN_4, L0);
					//set_dvfs_perf_level();
					touch_state_val=1;
					}
			#endif
...
#if defined(CONFIG_MACH_S5PC110_PRESTIGE)
static int qt602240_active_flag;

void qt602240_switch_suspend(void)
{
	int i=0;

	if (!qt602240_active_flag)
		return;

	mutex_lock(&qt602240_use);
	ENTER_FUNC;
	printk("[TSP][%s] \n",__func__);
	...
	LEAVE_FUNC;
	mutex_unlock(&qt602240_use);

	qt602240_active_flag = 0;

	return 0;
}

static void qt602240_switch_resume(void)
{
	int ret,i;
	...
	if (driver_setup == DRIVER_SETUP_OK)
	{
		read_mem(message_processor_address, 6, quantum_msg);
...
//			writel(readl(gpio_pend_mask_mem)|(0x1<<5), gpio_pend_mask_mem); 
	...
	qt602240_active_flag = 1;

	return 0;
}

void qt602240_reinit_tsp(void)
{
...
void qt602240_switch_sub2main_tsp(void)
{
	if (qt602240 == NULL)
		return;
...
//	qt602240_switch_suspend();
	...
	qt602240->client->addr = I2C_MAIN_TSP_ADDR;
	qt602240->working_tsp = QT602240_MAIN_TSP;

	s3c_gpio_setpin(GPIO_TSP_SEL, QT602240_MAIN_TSP);
	qt602240_reinit_tsp();
	qt602240_switch_resume();
}

void qt602240_switch_main2sub_tsp(void)
{
	if (qt602240 == NULL)
		return;
...
//	qt602240_switch_suspend();
	...
	qt602240->client->addr = I2C_SUB_TSP_ADDR;
	qt602240->working_tsp = QT602240_SUB_TSP;

	s3c_gpio_setpin(GPIO_TSP_SEL, QT602240_SUB_TSP);
	qt602240_reinit_tsp();
	qt602240_switch_resume();
}

void qt602240_set_working_tsp(int working_tsp)
{
	if (qt602240 == NULL)
		return;

	if (working_tsp == QT602240_MAIN_TSP)	{
		qt602240->client->addr = I2C_MAIN_TSP_ADDR;
		qt602240->working_tsp = QT602240_MAIN_TSP;
		s3c_gpio_setpin(GPIO_TSP_SEL, QT602240_MAIN_TSP);
	} else	{
...
#ifdef QT_FIRMUP_ENABLE
{
	...
	/* Read TSP reg if upper board connected */ // pje_CI16
	if (hw_upper_board_check())	{
		QT_reprogram();
	}
	...
	flip_status = (readl(S5PV210_GPH0DAT)) & (1 << 6);
	if(flip_status)	{	// MAIN
		qt602240_switch_suspend();
...
#define _Mxt224_ChannelMaxNum (16*14)
...
uint8_t read_dbg_all_data(uint8_t dbg_mode , uint8_t node, uint16_t * dbg_data)
{
   ...
   for(i=0;i<224;i++)
   {
		...
		if(dbg_mode ==QT_DELTA_MODE)
		{
			if(dbg_data[0] < 32767){
				//return sprintf(buf, "%u\n", dbg_data);
				//printk("i=%d, delta_data=%u before_read=%d\n", i, dbg_data[0], before_read_page);
			}
			else	{
				dbg_data[0] = 65535 - dbg_data[0];
				//return sprintf(buf, "-%u\n", dbg_data);	
				//printk("i=%d, delta_data=-%u before_read=%d\n", i, dbg_data[0], before_read_page);
			}
		}
		...
		{
			if((dbg_data[0]<5500) ||(dbg_data[0]>11500))
			{
				refer_fail_count++;
			}
			//printk("i=%d, reference_data=%u refer_fail_count=%d\n", i, dbg_data[0], refer_fail_count);
		}
		//msleep(5);
   }
   //printk("finish fail=%d\n", refer_fail_count);
   msleep(10); 
   #endif
   status = read_uint16_t_burst_page(diagnostic_addr, 0, dbg_data, _Mxt224_ChannelNumPerOnePage);
    ...
    status = read_uint16_t_burst_page(diagnostic_addr, 3, dbg_data+_Mxt224_ChannelNumPerOnePage*3, 32);
    msleep(15);

    for(i=0; i<_Mxt224_ChannelMaxNum; i++) 
    {
	    if((dbg_data[i]<5000) ||(dbg_data[i]>11500))
	    {
			refer_fail_count++;
	    }
		//printk("data[%d]=%d\n", i, dbg_data[i]);
...
static ssize_t set_refer_all_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	uint8_t status;
	uint16_t qt_refrence[_Mxt224_ChannelMaxNum];

#if defined(CONFIG_MACH_S5PC110_PRESTIGE)
	if(s3c_keygpio_get_flip_status() == 1)
		// FLIP CLOSE
...
static ssize_t set_delta_all_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if(s3c_keygpio_get_flip_status() == 1)
		// FLIP CLOSE
		status = read_dbg_all_data(QT_DELTA_MODE, 0, &qt_delta);
	else
#endif
...
#if 1 //jgblue.choi to check all reference data
static DEVICE_ATTR(set_refer, 0664, set_refer_all_mode_show, NULL);
static DEVICE_ATTR(set_delta, 0664, set_delta_all_mode_show, NULL);
#endif
