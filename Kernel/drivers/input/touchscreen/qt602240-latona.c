...
#define NOISE_FILTER	//for AT&T TA issue,factory noise, GB TSP test _ 20120215 by Tei.Lee
...
unsigned char test_node2[TEST_POINT_NUM] = {12,       		   20, 		   104,    		        188,     			   196};        
...
DEVICE_TYPE_MODE curent_device_type = DEVICE_TYPE_BATT;
t22_freq_table_config_t noise_freq_table = {0};

#endif

int touchkey_control(int data);

/* we do not use below variable, is_suspending */
/* do not turn off touchkey led here, but in function "s3cfb_early_suspend" at s3cfb.c */
...
void qt_Acquisition_Config_Init(void)
{
    ...
    acquisition_config.atchcalsthr = 15;//35;	// 35;
...
void qt_Multitouchscreen_Init(void)
{
    ...
    touchscreen_config.tchthr = 32;//40;//45;
...
void qt_Noise_Suppression_Config_Init(void)
{
	...
	/* Input Initial Values in frerr4 table. */
	noise_freq_table.freq_for_fherr4[0] = 20;
	noise_freq_table.freq_for_fherr4[1] = 30;
	noise_freq_table.freq_for_fherr4[2] = 35;
	noise_freq_table.freq_for_fherr4[3] = 45;
	noise_freq_table.freq_for_fherr4[4] = 55;
#endif	
}
...
void quantum_touch_probe(void)
{
	...
	else
	{
		...
		printk("[TSP] Family:		   0x%x\n\r", family_id);
		printk("[TSP] Variant: 	   0x%x\n\r", variant);
		printk("[TSP] Build number:   0x%x\n\r", build);

		printk("[TSP] Matrix X size : %d\n\r", info_block->info_id.matrix_x_size);
		printk("[TSP] Matrix Y size : %d\n\r", info_block->info_id.matrix_y_size);
...
//#define SEC_HEADSET_3_POLE_DEVICE		(0x01 << 1)
...
#ifdef NOISE_FILTER
		curent_device_type = DEVICE_TYPE_TA;

		if(noise_freq_table.t9_blen_for_fherr_cnt == 1)
		{
			touchscreen_config.tchthr = noise_freq_table.t9_thr_for_fherr;
			noise_suppression_config.noisethr = noise_freq_table.t22_noisethr_for_fherr;
		}
#endif
...
#ifdef NOISE_FILTER //20120215 add "if 1"
		curent_device_type = DEVICE_TYPE_BATT;
		gumi_err_setting(0);	
#endif	
		/* if TA or tv-out removed after both inserted */
		//if(maxim_chg_status()==1 || get_headset_status()==SEC_HEADSET_3_POLE_DEVICE)
			touchscreen_config.tchthr = 32;//40;
			noise_suppression_config.noisethr = 25;//30;
...
#ifdef NOISE_FILTER
		if ((noise_freq_table.fherr_setting==1) && (curent_device_type==DEVICE_TYPE_BATT)) 
		{
			...
			//tmp = &touchscreen_config.tchthr;
			//write_mem(object_address+7, 1, tmp);
...
void gumi_err_setting(int use)
{
	...
	if(use)
	{
		...
		value = 143;
		tmp = &value;
		write_mem(object_address+0, 1, tmp);

		value = 50;
		tmp = &value;
		write_mem(object_address+8, 1, tmp);

		value = 10;
		tmp = &value;
		write_mem(object_address+11, 1, tmp);

		value = 12;
		tmp = &value;
		write_mem(object_address+12, 1, tmp);
		...
		value = 20;
		tmp = &value;
		write_mem(object_address+14, 1, tmp);

		value = 25;
		tmp = &value;
		write_mem(object_address+15, 1, tmp);

		object_address = get_object_address(SPT_CTECONFIG_T28, 0);

		value = 24;
		tmp = &value;
		write_mem(object_address+4, 1, tmp);
		
/* Debug Original Register 	
		echo 0255 > set_power v
		echo 6032 > set_touchscreen
		echo 7055 > set_touchscreen
		echo 13080 > set_touchscreen
		echo 0143 > set_noise
		echo 6050 > set_noise
		echo 8010 > set_noise
		echo 9012 > set_noise
		echo 10018 > set_noise
		echo 11020 > set_noise
		echo 12025 > set_noise
		echo 7024 > set_total
*/
	}
	else
	{
		...
		tmp = &noise_suppression_config.freq[0];
		write_mem(object_address+11, 1, tmp);

		tmp = &noise_suppression_config.freq[1];
		write_mem(object_address+12, 1, tmp);

		tmp = &noise_suppression_config.freq[2];
		write_mem(object_address+13, 1, tmp);

		tmp = &noise_suppression_config.freq[3];
		write_mem(object_address+14, 1, tmp);

		tmp = &noise_suppression_config.freq[4];
		write_mem(object_address+15, 1, tmp);

		object_address = get_object_address(SPT_CTECONFIG_T28, 0);

		tmp = &cte_config.actvgcafdepth;
		write_mem(object_address+4, 1, tmp);
	}
}
...
static int freq_hop_err_setting(int state)
{
	...
	{
		object_address = get_object_address(TOUCH_MULTITOUCHSCREEN_T9, 0);
		data = noise_freq_table.t9_blen_for_fherr; // Gain 0
		...
		printk("[TSP] t9_blen_for_fherr_cnt == 0 \n");
	}
	...
	for (i = 0; i< 5; i++) {
		if (state == 1) {
			...
			data = noise_freq_table.freq_for_fherr4[i];
			write_mem(object_address+11+i, 1, &data);
		}
	}
...
void report_back_key(void)
{
	input_report_key(qt602240->input_dev, TOUCH_BACK, 1);
	input_report_key(qt602240->input_dev, TOUCH_BACK, 0);
	printk("report_back_key because of status changed\n");
}
EXPORT_SYMBOL(report_back_key);
...
static unsigned int did_autocal = 0;
			...
			if( quantum_msg[0] == 14 )
			{
				...
				{ 
					...
					for ( i= 0; i<MAX_USING_FINGER_NUM; ++i )
					{
						...
						if(i == 0){
//TEMP							s5pc110_unlock_dvfs_high_level(DVFS_LOCK_TOKEN_4);
							//set_dvfs_perf_level();
							touch_state_val=0;
							}
						fingerInfo[i].pressure= 0;
...
#ifdef NOISE_FILTER 
			else if (quantum_msg[0] == 15) 
			{ /* noise suppression */
				if ((quantum_msg[1]&0x08) == 0x08) 
				{
					switch(curent_device_type)
					{
						case DEVICE_TYPE_TA:
							// at & t
							...
							if (!(noise_freq_table.fherr_count%3)) //3 
							{
...
//TEMP					s5pc110_unlock_dvfs_high_level(DVFS_LOCK_TOKEN_4);
				...
				printk(KERN_DEBUG "[TSP]### F[%d] U - num is (%d)  status=0x%02x\n", id, --qt_touch_num_state[id], quantum_msg[1]);
			}
			...
			touch_message_flag = 1; //20100217 julia
				...
				if(id == 0){
					//set_dvfs_perf_level();
//TEMP					s5pc110_lock_dvfs_high_level(DVFS_LOCK_TOKEN_4, LEV_800MHZ);
					touch_state_val=1;
					}
			#endif
			#endif
				...
				printk(KERN_DEBUG "[TSP]### F[%d] D - num is (%d)   status=0x%02x\n", id, ++qt_touch_num_state[id], quantum_msg[1] );
			}
...
//TEMP				s5pc110_unlock_dvfs_high_level(DVFS_LOCK_TOKEN_4);
	...
	if(id>1 && did_autocal==0) {
		printk("check number of finger\n");
		acquisition_config.tchautocal = 15; // 3�� 200ms * 15 = 3s
		if (write_acquisition_config(acquisition_config) != CFG_WRITE_OK)
		{
			/* "Acquisition config write failed!\n" */
			dprintk("\n[TSP][ERROR] line : %d\n", __LINE__);
...
int qt602240_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	int ret;	
	int i;
	int key;
...
static int qt602240_early_suspend(struct early_suspend *h)
{
	...
	if(did_autocal) {
		printk("suspend check number of finger\n");
...
#if 0 // TSP off at LCD test binary
	gpio_direction_output(GPIO_TOUCH_EN, 0);
	//printk("gpio_direction_output0000\n");
#endif
...
static int qt602240_late_resume(struct early_suspend *h)
{
	...
	//printk("gpio_direction_output1111\n");
#endif
...
#if 1 // TSP off at LCD test binary
	if ( (ret = write_power_config(power_config)) != CFG_WRITE_OK)
	{
		/* "Power config write failed!\n" */
		for(i=0;i<10;i++) {
		...
		const char *buf, size_t size)
{
	...
	if(sscanf(buf,"%d",&i)==1)
	{
    		touchscreen_config.tchthr = i;	//basically,26;
...
int touchkey_control(int data)
{
	/* lcd dimming and touchkey led turns off at debug screen */
	/* if health is not good, do not turn touchkey led on */
	#if 0
	if(is_debug_screen || cable_status==3)
		data = 2;

	/* if lcd is off, touchkey led does not turn on */
	if(IsLDIEnabled()==0)
		return 1;
	#endif
...
EXPORT_SYMBOL(touchkey_control);
...
static DEVICE_ATTR(brightness, S_IRUGO | S_IWUSR, NULL, touchkey_led_store);
...
static DEVICE_ATTR(set_power, S_IRUGO | S_IWUSR | S_IWGRP, set_power_show, set_power_store);
...
static DEVICE_ATTR(set_acquisition, S_IRUGO | S_IWUSR | S_IWGRP, set_acquisition_show, set_acquisition_store);
...
static DEVICE_ATTR(set_touchscreen, S_IRUGO | S_IWUSR | S_IWGRP, set_touchscreen_show, set_touchscreen_store);
...
static DEVICE_ATTR(set_keyarray, S_IRUGO | S_IWUSR | S_IWGRP, set_keyarray_show, set_keyarray_store);
...
static DEVICE_ATTR(set_qt_init_state, S_IRUGO | S_IWUSR | S_IWGRP, set_qt_init_state_show, NULL);
...
static DEVICE_ATTR(set_noise, S_IRUGO | S_IWUSR | S_IWGRP, set_noise_show, set_noise_store);
...
static DEVICE_ATTR(set_grip, S_IRUGO | S_IWUSR | S_IWGRP, set_grip_show, set_grip_store);
...
static DEVICE_ATTR(set_total, S_IRUGO | S_IWUSR | S_IWGRP, set_total_show, set_total_store);
...
static DEVICE_ATTR(set_write, S_IRUGO | S_IWUSR | S_IWGRP, set_write_show, set_write_store);
...
static ssize_t set_refer0_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	status = read_dbg_data(QT_REFERENCE_MODE, test_node2[0],&qt_refrence);
	return sprintf(buf, "%u\n", qt_refrence);	
}
...
static ssize_t set_refer1_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	status = read_dbg_data(QT_REFERENCE_MODE, test_node2[1],&qt_refrence);
	return sprintf(buf, "%u\n", qt_refrence);	
}
...
static ssize_t set_refer2_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	status = read_dbg_data(QT_REFERENCE_MODE, test_node2[2],&qt_refrence);
	return sprintf(buf, "%u\n", qt_refrence);	
}
...
static ssize_t set_refer3_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	status = read_dbg_data(QT_REFERENCE_MODE, test_node2[3],&qt_refrence);
	return sprintf(buf, "%u\n", qt_refrence);	
}
...
static ssize_t set_refer4_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	status = read_dbg_data(QT_REFERENCE_MODE, test_node2[4],&qt_refrence);
	return sprintf(buf, "%u\n", qt_refrence);	
}
...
static ssize_t set_delta0_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	status = read_dbg_data(QT_DELTA_MODE, test_node2[0],&qt_delta);
	if(qt_delta < 32767){
		return sprintf(buf, "%u\n", qt_delta);	
	   }
	else	{
...
static ssize_t set_delta1_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	status = read_dbg_data(QT_DELTA_MODE, test_node2[1],&qt_delta);
	if(qt_delta < 32767){
		return sprintf(buf, "%u\n", qt_delta);	
	   }
	else	{
...
static ssize_t set_delta2_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	status = read_dbg_data(QT_DELTA_MODE, test_node2[2],&qt_delta);
	if(qt_delta < 32767){
		return sprintf(buf, "%u\n", qt_delta);	
	   }
	else	{
...
static ssize_t set_delta3_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    status = read_dbg_data(QT_DELTA_MODE, test_node2[3],&qt_delta);
	if(qt_delta < 32767){
		return sprintf(buf, "%u\n", qt_delta);	
	   }
	else	{
...
static ssize_t set_delta4_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    status = read_dbg_data(QT_DELTA_MODE, test_node2[4],&qt_delta);
	if(qt_delta < 32767){
		return sprintf(buf, "%u\n", qt_delta);	
	   }
	else	{
...
#if 1 //jgblue.choi to check all reference data
#define _Mxt224_ChannelNumPerOnePage 64
#define _Mxt224_ChannelMaxNum (18*12)
uint8_t read_uint16_t_burst_page( uint16_t Address, uint8_t PageIndex, uint16_t *Data, uint16_t DataSize)
{
    uint8_t temp[256];
    uint8_t Status, Loop=0;
    uint8_t page_info[2];

    // Read 2 bytes to check page info. Info[0] = line number, Info[1] = Page number
    Status = read_mem(Address, 2, page_info);
    for(;;)
    {
        if((PageIndex == page_info[1]) || (Loop++ == 200))
        {
            break;
        }
        msleep(5);
    ...
    // Read DataSize*2 bytes
    Status = read_mem(Address+2, DataSize*2, temp);

    for(Loop=0; Loop<DataSize; Loop++)
    {
        *Data = ((uint16_t)temp[Loop*2+1] << 8) + (uint16_t)temp[Loop*2];
        Data++;
    }
...
uint8_t read_dbg_all_data(uint8_t dbg_mode , uint8_t node, uint16_t * dbg_data)
{
    uint8_t status;
    uint8_t mode,page,i;
    uint8_t read_page,read_point,before_read_page=0;
    uint8_t refer_fail_count=0;
   ...
   printk("start\n");
    ...
    status = read_uint16_t_burst_page(diagnostic_addr, 0, dbg_data, _Mxt224_ChannelNumPerOnePage);
    // Move to proper page number
    status = diagnostic_chip(QT_PAGE_UP);
    msleep(15);

    status = read_uint16_t_burst_page(diagnostic_addr, 1, dbg_data+_Mxt224_ChannelNumPerOnePage, _Mxt224_ChannelNumPerOnePage);
    // Move to proper page number
    status = diagnostic_chip(QT_PAGE_UP);
    msleep(15);

    status = read_uint16_t_burst_page(diagnostic_addr, 2, dbg_data+_Mxt224_ChannelNumPerOnePage*2, _Mxt224_ChannelNumPerOnePage);
    // Move to proper page number
    status = diagnostic_chip(QT_PAGE_UP);
    msleep(15);

    status = read_uint16_t_burst_page(diagnostic_addr, 3, dbg_data+_Mxt224_ChannelNumPerOnePage*3, 24);
    msleep(15);

#if 0
	for(i=0 ; i<_Mxt224_ChannelMaxNum ; i++)
	{
		printk("show all ref[%d]=%d\n", i, dbg_data[i]);
	}
#endif
     return refer_fail_count;
}
#endif
...
{
	uint8_t status;
	uint16_t qt_refrence[_Mxt224_ChannelMaxNum];
	int i = 0;
	int buf_plus = 0;
	
	status = read_dbg_all_data(QT_REFERENCE_MODE, 0, qt_refrence);

	for(i=0 ; i<_Mxt224_ChannelMaxNum ; i++) 
		buf_plus += sprintf(buf + buf_plus, "%d ", qt_refrence[i]);

	return buf_plus+sizeof(uint16_t);
}
...
static DEVICE_ATTR(set_refer, S_IRUGO | S_IWUSR | S_IWGRP, set_refer_mode_show, set_refer_mode_store);
static DEVICE_ATTR(set_all_refer, S_IRUGO | S_IWUSR | S_IWGRP, set_all_refer_mode_show, NULL);
static DEVICE_ATTR(set_delta, S_IRUGO | S_IWUSR | S_IWGRP, set_delta_mode_show, set_delta_mode_store);
...
//	quantum_touch_probe();  /* find and initialise QT device */
		...
		printk("[TSP]Reprogram done : Firmware update Fail~~~~~~~~~~\n");
	}
	TSP_Restart();
        quantum_touch_probe();
//	msleep(100);
...
static DEVICE_ATTR(set_tsp_for_inputmethod, S_IRUGO | S_IWUSR | S_IWGRP, set_tsp_for_inputmethod_show, set_tsp_for_inputmethod_store);
...
static DEVICE_ATTR(call_release_touch, S_IRUGO | S_IWUSR | S_IWGRP, qt602240_call_release_touch, NULL);
...
int __init qt602240_init(void)
{
	...
	printk("[TSP] ret : %d, qt602240->client name : %s\n",ret,qt602240->client->name);
	...
	printk("[TSP] %s/%d, platform_driver_register!!\n",__func__,__LINE__);
