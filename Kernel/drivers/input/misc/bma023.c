...
static int bma023_sleepin_flag = 0;		// for ignoer enable_store after suspend.
static int bma023_first_work = 1;		// for first work start
static void bma023_set_enable(struct device *dev, int enable)
{
	struct bma023_data *bma023 = dev_get_drvdata(dev);
	int delay = atomic_read(&bma023->delay);
	printk("[BMA] %s - enable: %d \n",__func__, enable);
#if 0	// ignoerd framework control, always on in idle.
...
#else
	if(bma023_first_work){
		bma023_first_work = 0;
		
		printk("[BMA] %s - first: %d \n",__func__, bma023_first_work);
		mutex_lock(&bma023->enable_mutex);
		bma023_power_up(bma023);
		schedule_delayed_work(&bma023->work, delay_to_jiffies(delay) + 1);		
        mutex_unlock(&bma023->enable_mutex);
	}
				...
				struct acceleration *accel)
{
	...
	if (err < 0) {
		pr_err("[BMA] %s: i2c read fail addr=0x%02x, len=%d\n",
...
static void bma023_work_func(struct work_struct *work)
{
	...
	struct acceleration accel;
	struct acceleration tempAccel;
	unsigned long delay = delay_to_jiffies(atomic_read(&bma023->delay));

	bma023_measure(bma023, &tempAccel);
#if defined(CONFIG_MACH_S5PC110_LATONA) || defined(CONFIG_MACH_S5PC110_GALAXY) //I909GB and I809GB
	//change position
	accel.x = tempAccel.y;
	accel.y = tempAccel.x;
	accel.z = -tempAccel.z;
	...
	// pr_err("[BMA] %s: work_function call\n", __FILE__);
				   ...
				   const char *buf, size_t count)
{
	...
	if ((enable == 0) || (enable == 1)) {
		/* do not disable, it consume more current */
		if(bma023_sleepin_flag){
			printk("[BMA] %s , ignoerd sleep:%d \n",__func__,bma023_sleepin_flag);
		}else{
			enable = 1;
...
#ifdef CONFIG_HAS_EARLYSUSPEND
static void bma023_early_suspend(struct early_suspend *handler)
{	
	struct bma023_data *data ;
	data = container_of(handler, struct bma023_data, early_suspend);
	
	printk("[BMA] %s + \n",__func__);
	bma023_set_enable(&data->client->dev, 0);
	bma023_sleepin_flag = 1;

	mutex_lock(&data->enable_mutex);
	cancel_delayed_work_sync(&data->work);
	bma023_power_down(data);
	mutex_unlock(&data->enable_mutex);

	printk("[BMA] %s - \n",__func__);
}

static void bma023_early_resume(struct early_suspend *handler)
{
	struct bma023_data *data ;
	data = container_of(handler, struct bma023_data, early_suspend);
	...
	printk("[BMA] %s + \n",__func__);
	bma023_hw_init(data);
	bma023_set_delay(&data->client->dev, delay);

	mutex_lock(&data->enable_mutex);
	/*had to direct power up , because in suspend mode directly setting power down*/
	//if (bma023_get_enable(&data->client->dev)) {
	bma023_power_up(data);
	schedule_delayed_work(&data->work, delay_to_jiffies(delay) + 1);
	//}
	mutex_unlock(&data->enable_mutex);

	bma023_set_enable(&data->client->dev, 1);
	bma023_sleepin_flag = 0;
	printk("[BMA] %s - \n",__func__);
...
#ifdef CONFIG_HAS_EARLYSUSPEND		
       bma023->early_suspend.suspend = bma023_early_suspend; 	
       bma023->early_suspend.resume = bma023_early_resume;		
       register_early_suspend(&bma023->early_suspend);	
#endif

	err = misc_register(&bma023->bma023_device);
	if (err) {
		pr_err("[BMA] %s: misc_register failed\n", __FILE__);
...
static int bma023_suspend(struct device *dev)
{
	...
	/* it should be disable while suspend */
	bma023_set_enable(dev, 0);
