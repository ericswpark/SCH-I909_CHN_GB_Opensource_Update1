...
u16 who_is_the_caller_of_fsa9480 = CALL_FROM_USB_CABLE; // by kbj , 0 means that it is called from FSA9480. 1 == from USBMenuSel
u16 who_is_the_caller_of_usbmenusel = CALL_FROM_USB_SETTING; // by kbj , 0 means that it is called from Setting and Init.rc . 1 == from AskUsbMode  
#endif
...
extern int fsa9480_is_bootusb(void); 
static void samsung_enable_function(int mode)
{
	struct android_dev *dev = _android_dev;
	int product_id = 0;
	...
	printk("%s , askonstatus=%x , mode=%x , inaskonstatus=%x , who_is_the_caller=%x \n", __func__ , askonstatus , mode , inaskonstatus , who_is_the_caller_of_fsa9480 );

#if defined(CONFIG_MACH_S5PC110_PRESTIGE) || defined(CONFIG_MACH_S5PC110_GALAXY) || defined(CONFIG_MACH_S5PC110_LATONA)// kbj	 for the ASK on menu
	if( askonstatus == 0xabcd && who_is_the_caller_of_usbmenusel != CALL_FROM_USB_SETTING )
	{
		if(mode == USBSTATUS_ASKON)
			return;
		who_is_the_caller_of_fsa9480 = CALL_FROM_USB_MENU_SEL;
		fsa9480_usb_cb(1);		
	}
	...
	switch(mode) {
		case USBSTATUS_UMS:
			...
			ret = set_product(dev, USBSTATUS_UMS);
#if defined(CONFIG_MACH_S5PC110_PRESTIGE) || defined(CONFIG_MACH_S5PC110_GALAXY) || defined(CONFIG_MACH_S5PC110_LATONA) // kbj	 for the ASK on menu
			if( who_is_the_caller_of_usbmenusel == CALL_FROM_USB_SETTING )
			{
				askonstatus = 0;
				fsa9480_usb_cb( fsa9480_is_bootusb() );
			}
#endif
			break;
...
static ssize_t UsbMenuSel_switch_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	switch(value) {
        ...
        case 4:
            ...
            break;
		case 8:
			CSY_DBG_ESS("From Setting Menu)\n");	
			who_is_the_caller_of_usbmenusel = CALL_FROM_USB_SETTING;		
		break;
		case 9:
			CSY_DBG_ESS("From AskOn Menu\n");
			who_is_the_caller_of_usbmenusel = CALL_FROM_ASKON_MENU;
		break;
