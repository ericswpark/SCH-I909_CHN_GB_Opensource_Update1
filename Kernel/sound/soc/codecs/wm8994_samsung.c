...
static const char *mic_path[] = {
	"Main Mic", "Hands Free Mic", "BT Sco Mic", "Voice", "MIC OFF"
};
...
/* CTC_MAIN_soon9.lee - start */
static const char *pcm_path_control[] = {"CDMA", "GSM"};

static const char *submic_status[] = {"OFF", "ON"};
/* CTC_MAIN_soon9.lee - end */
...
/* CTC_MAIN_soon9.lee - start */		
	case 3:
		wm8994->rec_path = VOICE;
		wm8994->voice_path = VOICE_OFF;
		break;
/* CTC_MAIN_soon9.lee - end */		
	case 4:
...
/* CTC_MAIN_soon9.lee - start */
	...
	case CMD_CODEC_EMERGENCY_RECOVERY:
		...
		msleep(80); /* CTC_MAIN_soon9.lee - start */
...
/* CTC_MAIN_soon9.lee - start */
static int wm8994_get_submic_status(struct snd_kcontrol *kcontrol, 
	struct snd_ctl_elem_value *ucontrol)
{	
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct wm8994_priv *wm8994 = codec->drvdata;

	DEBUG_LOG("Now SubMic Status= [%d]", wm8994->use_submic);

	return wm8994->use_submic;
}

static int wm8994_set_submic_status(struct snd_kcontrol *kcontrol, 
	struct snd_ctl_elem_value *ucontrol)
{
	int ret;
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	...
	int new_submic_status = ucontrol->value.integer.value[0];	

	DEBUG_LOG("Now Submic Status is set =[%d]", new_submic_status); 

	if(wm8994->use_submic == new_submic_status)
	{
		printk("new submic status is same with old submic status\n");
		return 0;
	}

	wm8994->use_submic = new_submic_status;

	return ret;
}

static int wm8994_get_pcm_path(struct snd_kcontrol *kcontrol, 
	struct snd_ctl_elem_value *ucontrol)
{	
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct wm8994_priv *wm8994 = codec->drvdata;

	DEBUG_LOG("Now PCM Path is = [%d]", wm8994->pcm_path);

	return wm8994->pcm_path;
}

static int wm8994_set_pcm_path(struct snd_kcontrol *kcontrol, 
	struct snd_ctl_elem_value *ucontrol)
{
	int ret;
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	...
	int new_pcm_path = ucontrol->value.integer.value[0];	

	DEBUG_LOG("Now PCM Path is set =[%d]", new_pcm_path); 

	if(wm8994->pcm_path == new_pcm_path)
	{
		printk("new pcm path is same with old pcm path\n");
		return 0;
	}

	ret = wm8994_switch_pcm_path(codec, new_pcm_path);
	wm8994->pcm_path = new_pcm_path;
	wm8994->clock_change = 1;
				 ...
				 struct snd_ctl_elem_value *ucontrol)
{
	...
	if (wm8994->clock_change || wm8994->cur_path != path_num ||
			!(wm8994->codec_state & CALL_ACTIVE)
...
/* CTC_MAIN_soon9.lee - start */
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(pcm_path_control), pcm_path_control),
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(submic_status), submic_status),
/* CTC_MAIN_soon9.lee - end */	
};
...
/* CTC_MAIN_soon9.lee - start */
	SOC_ENUM_EXT("Voice Call PCM Path", path_control_enum[8],
						wm8994_get_pcm_path, wm8994_set_pcm_path),
	SOC_ENUM_EXT("Submic Status", path_control_enum[7],
						wm8994_get_submic_status, wm8994_set_submic_status),						
/* CTC_MAIN_soon9.lee - end */		
};
...
/* CHN soon9.lee - start */	
	//wm8994->recognition_active = REC_OFF;
	wm8994->voice_path = VOICE_OFF;	
	wm8994->pcm_path = PCM_OFF;
	wm8994->clock_change = 0; 
/* CHN soon9.lee - end */
	wm8994->pdata = pdata;
