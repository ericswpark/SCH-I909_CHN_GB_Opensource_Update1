...
check_partition(struct gendisk *hd, struct block_device *bdev)
{
	...
	if (!state) {
		...
		if (!state)
		{
			printk(KERN_INFO " The request size is = %d \n" , sizeof(struct parsed_partitions));
			printk(KERN_INFO " The trial to allocation memory by vmalloc fails! \n");
			return NULL;
		}
		else
		{	
			printk(KERN_INFO " The request size is = %d \n" , sizeof(struct parsed_partitions));
			printk(KERN_INFO " The addr of state = %x! \n" , state);
		}
