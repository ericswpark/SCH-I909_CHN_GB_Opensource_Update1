...
bool RenderBlock::handleRunInChild(RenderBox* child)
{
	...
	//eunice : Google CTS 
	 // First we destroy any :before/:after content. It will be regenerated by the new inline. 	 // Exception is if the run-in itself is generated.
...
VisiblePosition RenderBlock::positionForPoint(const IntPoint& point)
{
    ...
    if (isReplaced()) {
        ...
        if (point.y() >= (height() + 5)|| (point.y() >= 0 && point.x() >= width()))	// added 5px error margin for height GA0100267731
